...
var removeAFeature = (jsonFile, featureName) => {
  ...
  fs.writeFileSync(
    "removed_" + path.basename(jsonFile),
...
var cutAHoleInFeatureAWithFB = (jsonFile, featureA, featureB) => {
  ...
  var geojson = JSON.parse(data);
  var featurea = geojson.features.find(
    (feature) => feature.properties.name === featureA,
  );
  var featureb = geojson.features.find(
    (feature) => feature.properties.name === featureB,
  );
...
var cutAHoleInGeojsonByOther = (jsonFile, jsonFile2) => {
  ...
  geojson.features[0].geometry.coordinates.push(
    second.features[0].geometry.coordinates[0],
  );
...
var geoJsonMergeTwoPropertiesAsOne = (
  jsonFile,
  propertyA,
  propertyB,
  newProperty,
  ...
  geojson.features.forEach(function (feature) {
    feature.properties[newProperty] =
      feature.properties[propertyA] + " " + feature.properties[propertyB];
